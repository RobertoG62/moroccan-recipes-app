...
const RecipeData = (() => {
    ...
    async function fetchRecipes(lang) {
        ...
        state.categories = [state.allLabel, ...new Set(state.recipes.map(r => r.category))];
        state.isLoaded = true;
        return state;
    }
    ...
    function filterRecipes(query, category) {
        ...
        state.activeCategory = category || state.allLabel;

        state.filteredRecipes = state.recipes.filter(recipe => {
            const matchesCategory = state.activeCategory === state.allLabel || recipe.category === state.activeCategory;

...
const UI = (() => {
    // Moroccan cuisine categories with icons
    const CATEGORY_ICONS = {
        // Hebrew
        'הכל': 'fa-utensils',
        'סלטים וממרחים': 'fa-jar',
        'מרקים וחרירה': 'fa-bowl-food',
        'טאג׳ין ובשרים': 'fa-fire-burner',
        'קוסקוס ומאפים': 'fa-wheat-awn',
        'קינוחים ומתוקים': 'fa-cake-candles',
        // English
        'All': 'fa-utensils',
        'Salads & Dips': 'fa-jar',
        'Soups & Harira': 'fa-bowl-food',
        'Tagines & Meats': 'fa-fire-burner',
        'Couscous & Pastries': 'fa-wheat-awn',
        'Desserts & Sweets': 'fa-cake-candles',
    };

    // Kashrut category. Every recipe on this site is kosher; the badge says which
    // of the three groups it belongs to, so a cook knows what it can be served with.
    const KOSHER_BADGES = {
    ...
    function renderCards(recipes) {
        ...
        grid.innerHTML = recipes.map(recipe => `
            <article class="recipe-card card-lift" data-id="${recipe.id}">
                ...
                <div class="p-4">
                    <h3 class="font-bold text-lg mb-0.5 text-ma-charcoal">${recipe.title}</h3>
                    ${recipe.originalName ? `<p class="text-ma-primary-dark text-xs italic mb-2 font-playfair">${recipe.originalName}</p>` : ''}
                    <p class="text-ma-text-secondary text-sm line-clamp-2 mb-3">${recipe.description}</p>
                    <div class="flex items-center justify-between text-xs text-ma-text-secondary">
            ...
            <div class="view-fade-in">
                <div class="recipe-hero" style="background-image: url('${recipe.image}')">
                    <div class="relative z-10 w-full p-6 md:p-10 max-w-4xl mx-auto">
                        <span class="inline-block px-3 py-1 rounded-full text-xs font-medium bg-ma-primary text-white mb-3">
                        ...
                        ${recipe.originalName ? `<p class="text-ma-primary-light text-base md:text-lg italic font-playfair mt-1">${recipe.originalName}</p>` : ''}
                ...
                <div class="max-w-4xl mx-auto px-4 -mt-8 relative z-10">
                    <div class="glass rounded-xl p-4 flex justify-center gap-0 mb-8">
                        <div class="meta-item">
                            <i class="far fa-clock text-ma-primary"></i>
                            <span class="font-bold text-ma-charcoal">${recipe.prepTime} ${i18n.t('detail.minutes')}</span>
                            <span class="text-xs text-ma-text-secondary">${i18n.t('detail.prepTime')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-fire text-ma-primary"></i>
                            <span class="font-bold text-ma-charcoal">${recipe.cookTime} ${i18n.t('detail.minutes')}</span>
                            <span class="text-xs text-ma-text-secondary">${i18n.t('detail.cookTime')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-users text-ma-primary"></i>
                            <span class="font-bold text-ma-charcoal">${recipe.servings}</span>
                            <span class="text-xs text-ma-text-secondary">${i18n.t('detail.servings')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-gauge text-ma-primary"></i>
                            <span class="font-bold text-ma-charcoal">${recipe.difficulty}</span>
                            <span class="text-xs text-ma-text-secondary">${i18n.t('detail.difficulty')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas ${getKosherBadge(recipe.kosherType).icon} text-ma-primary"></i>
                            <span class="font-bold text-ma-charcoal">${recipe.kosherType}</span>
                            <span class="text-xs text-ma-text-secondary">${i18n.t('detail.kosher')}</span>
                        </div>
                    </div>

                    <div class="glass gold-border-right rounded-xl p-6 mb-8">
                        <h2 class="font-bold text-xl mb-4 text-ma-charcoal flex items-center gap-2">
                            <i class="fas fa-list text-ma-primary"></i>
                            ${i18n.t('detail.ingredients')}
                            <span class="text-sm font-normal text-ma-text-secondary">(${recipe.ingredients.length})</span>
                        </h2>
                        <div>
                            ${recipe.ingredients.map(ing => `
                                <div class="ingredient-row">
                                    <span class="text-ma-primary"><i class="fas fa-check text-xs"></i></span>
                    ...
                    <div class="mb-8">
                        <h2 class="font-bold text-xl mb-6 text-ma-charcoal flex items-center gap-2">
                            <i class="fas fa-list-ol text-ma-primary"></i>
                        ...
                        <div class="space-y-4">
                            ${recipe.instructions.map((step, i) => `
                                <div class="flex gap-4 items-start">
                                    ...
                                    <p class="text-ma-charcoal pt-1 leading-relaxed">${mdBold(step)}</p>
                                </div>
                            `).join('')}
                        </div>
                    </div>
